...
export async function POST(request: NextRequest) {
  try {
    ...
    const rl = rateLimit({ key: `business-contact:${ip}`, limit: 5, windowMs: 10 * 60 * 1000 });
    if (!rl.ok) {
    ...
    if (String(companyName).length > 200 || String(message).length > 5000) {
      return NextResponse.json(
    ...
    const { data, error } = await resend.emails.send({
      ...
      subject: `【ビジネスお問い合わせ】${esc(companyName)}`,
      html: `
          ...
          <div style="background-color: #f9f9f9; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                ...
                <td style="padding: 8px 0;">${esc(companyName)}</td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td style="padding: 8px 0;">${phone ? esc(phone) : '未入力'}</td>
              </tr>
            </table>
